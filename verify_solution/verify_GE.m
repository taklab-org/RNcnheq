...
% delta3 = 2*(rho*(rc+rho*rs)+rs); % checked by defining rho
delta4 = 2*(rc+2*rho*rs+rs); if delta4 >= mu, success = 0; return, end
epsilon1 = (2/(mu-delta1))*( 2*rc+rho*rs/2+4*rc*rs*rho/(mu-delta2)+rs^2*(1+rho^2)/(2*mu-delta1-delta2)  )
lambda = 4*(rho*(rc+rho*rs)+rs)*rs/((mu-delta1)*(mu-delta4)) + 2*(rc+rho*rs)/(mu-delta1)

% 
if mu>delta2+2*rc && epsilon1<1 && lambda<1 && rho*rs < min(0.02*rs, -real(exp(1i*theta)*ac)) % it is assumed that rc := |a_c| + 0.02*|a_s| and r_s := |a_s|
  success = 1;
  disp('success for GE')
else
  success = 0;
